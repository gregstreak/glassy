...
- Rain probability: ${w.rainProb != null ? w.rainProb + '%' : 'unavailable'}
          ...
          {phase === 'done' && r && w && (
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 2 }}>
                ...
                {/* Refresh button */}
                <button onClick={refresh} title="Refresh conditions"
                  style={{ background: 'transparent', border: 'none', color: MUTED, cursor: 'pointer', padding: '2px 4px', fontSize: 16, lineHeight: 1 }}>
                  ↻
                </button>
              </div>
                  ...
                  <div style={{ display: 'flex', gap: 5, overflowX: 'auto', paddingBottom: 2 }}>
                    {r.trajectory.map((t, i) => (
                      <div key={i} style={{ background: SURFACE, borderRadius: 6, padding: '7px 9px', minWidth: 56, textAlign: 'center', flexShrink: 0 }}>
                        ...
                        {m && t.waveHeight != null && (
                          <div style={{ fontSize: 12, fontFamily: 'monospace', color: TEXT, marginTop: 2 }}>
                            {f1(t.waveHeight)}<span style={{ fontSize: 9, color: MUTED }}>m</span>
                          </div>
                        )}
                        <div style={{ fontSize: 12, fontFamily: 'monospace', color: MUTED, marginTop: 2 }}>
                          {f0(t.windSpeed)}<span style={{ fontSize: 9, color: MUTED }}>k</span>
                        </div>
                        {t.rainProb != null && (
                          <div style={{ fontSize: 10, color: t.rainProb > 50 ? AMBER : MUTED, marginTop: 2 }}>
                            {t.rainProb}<span style={{ fontSize: 8 }}>%</span>
                          </div>
                        )}
                      </div>
                    ))}
              ...
              <div style={{ marginTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <button onClick={refresh} disabled={phase === 'loading'}
                  style={{ background: 'transparent', border: `1px solid ${AMBER}55`, borderRadius: 6, color: AMBER, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '5px 12px', cursor: 'pointer', opacity: phase === 'loading' ? 0.4 : 1 }}>
                  ↻ Refresh
                </button>
                <button onClick={reset}
                  style={{ background: 'transparent', border: '1px solid #2A3A54', borderRadius: 6, color: MUTED, fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '5px 10px', cursor: 'pointer' }}>
                  New location
                </button>
              </div>
            </div>
