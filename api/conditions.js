export default async function handler(req, res) {
  ...
  try {
    const [metRes, marineRes, sstRes, precipRes] = await Promise.all([
      ...
      fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=precipitation_probability&timezone=auto&forecast_days=3&past_hours=48`)
    ]);
